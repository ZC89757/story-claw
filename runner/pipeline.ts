...
1. 对原文中每一个角色：
   - 不在已有列表中：用 write 工具写角色 JSON，记入 new_characters
   - 已在列表中：用 resource_detail 检查是否有新造型阶段，有则更新 JSON 并记入 existing_character_stages，无则跳过
...
export async function archive(sel: NovelSelection, visualPresetPath: string): Promise<ArchiveResult> {
  ...
  const taskPrompt = [
    ...
    listText,
    ``,
    `角色 JSON 存储目录：${charsDir}（每个角色写一个 {角色名}.json 文件）`,
    ``,
    `请根据以上信息，分析「${sel.novelName}」本章涉及的角色和场景，完成角色 JSON 写入，然后输出 archive-tasks JSON 块。`,
  ].join("\n");
