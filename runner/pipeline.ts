...
- video_prompt：基于 image_prompt 画面的意象动态，英文，一律固定镜头，以 "Fixed camera," 起头（不使用推镜/拉镜等任何运镜）。不要重复 image_prompt 的静态描述，只补画面内意象的细微动态。例外：当 image_prompt 属于图表/统计/截图类（信息图、排行榜、对比柱状图、价格表、流程图、UI 截图、文档/界面截图等静态信息载体）时，**不输出 video_prompt 字段**——这类画面无动态，render 侧会用静态图填满时长，无需生成视频。数学模型类画面同理：仅当语义必须靠变量变化才体现（如"随 x 增大而收敛""曲面随参数形变"）时才输出 video_prompt，只描述该变量的连续变化；若模型本身静态即可说明问题，则不输出 video_prompt。
