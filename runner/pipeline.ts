...
- append_group 的 content 参数是单个 group 的 JSON 字符串，格式：
  {"text":"...","end_positions":"含人物位置、朝向及人物与环境事物/彼此的相对位置关系","panels":[{"shot_type":"...","is_continuation":false,"image_prompt":"...","video_prompt":"..."}]}
...
export async function assignGlobalOrder(
  ...
  const groups: Array<{ scene: string; gi: number; text: string; globalOrder: number }> = [];
  ...
  for (const sceneName of sceneNames) {
    ...
    for (let gi = 0; gi < lines.length; gi++) {
      try {
        ...
        if (!text) continue;
        groups.push({ scene: sceneName, gi, text, globalOrder: -1 });
  ...
  for (const group of groups) {
    ...
    group.globalOrder = bestMatch >= 0 ? bestMatch : 999;
    if (bestMatch < 0) {
      console.warn(`[assignGlobalOrder] 未匹配: scene=${group.scene} gi=${group.gi} text="${group.text.slice(0, 60)}"`);
    }
  }
