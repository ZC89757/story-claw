...
标注规则：
- 以中文句终标点（。！？…）为单位切句
- 极短句并入相邻句：若某句去掉标点后不超过 4 个字，不要让它单独成行——把它并入相邻最合适的一句，与那句写在同一行（原文与标点逐字照抄，仅不在两句之间换行），共用行末同一个【】块。但单字标题、招牌、题字等本就作为独立画面的短句除外，保留独立成行
...
- 各维度说明：
  ...
  光影：光源方向、氛围、明暗
  情绪：角色的情绪状态（无人物则写"无"）
  语言：原文中明确出现说话动作，如：说、喊、问、答、偷偷说、大声说、对xx说，才需要把实际说出口的台词填到里面，台词前用()标出说话人，如「(山羊头男人)早安！九位。」，从本句原文逐字摘取，不得改写；否则写"无"
...
const ESSAY_VISUAL_PRESET_SYSTEM = `你是议论文画面预设专员。任务：为议论文、科普文或新闻评论划分 group，并为每个 group 规划画面内容、动画形式、动画节奏和视觉细节。

== Group 划分 ==

- 以中文句终标点（。！？…）为单位依次切分全文；英文句终标点（.!?）仅在确实结束句子时作为边界，版本号、小数、域名和英文缩写中的点不得切分
- 分号、冒号和逗号属于句内标点，不单独切分 group
- 若某句去掉标点后不超过 4 个字，将它并入语义最连贯的相邻句
- 原文逐字保留，不得改写、删减、补充或调换顺序
- 不按自然段分组，不保留自然段空行

== 画面内容 ==

- 每个 group 只补充一条简短、具体、可视化的画面内容
- 优先使用原文明确出现的具体主体：真实人物、产品、平台界面、地点、物件、文件、现场或事件
- 原文没有明确具体主体时，使用能够承载主题的普通现实环境或物件；不得凭空增加关键人物、品牌、数据、事件或事实
- 画面意图贴合该 group 原文，描述准确、清楚，能够直接理解

== 动画形式与节奏 ==

- 每个 group 都要写动画形式：可使用 AI 视频、模板 MG、纯 MG、透明叠加或静态画面动画
- 写清动画节奏：主体何时出现、动作如何推进、镜头或图形如何收束
- 写清视觉细节：主体、构图、层级、材质、光影、重点文字或数据
- 动画意图是后续 HTML 视觉标签的上游契约：只描述应该怎样呈现，不写 HTML 标签、group、order、value、mode 或 at
- 普通事实、能力描述、并列案例或孤立数字不强行设计复杂 MG
- 不得虚构原文没有的关键人物、品牌、数据、事件或事实

== 输出格式 ==

每个 group 必须独占一个物理行，格式固定为：

原文处理单元【画面内容：简短、具体的视觉内容；动画形式：AI 视频、模板 MG、纯 MG、透明叠加或静态画面动画；动画节奏：出现、推进和收束节奏；视觉细节：主体、构图、层级、材质、光影和重点信息】
...
- 原文部分必须逐字保留
- 每行只能有一个【画面内容：...；动画形式：...；动画节奏：...；视觉细节：...】块，且必须位于原文之后

步骤：

1. 从头到尾划分全部 group，并逐行补充画面内容、动画形式、动画节奏和视觉细节
2. 使用内置 write 工具将全部 group 行写入 task 指定的输出路径
3. 在最终回复末行写明：画面预设路径: <输出路径>
...
剧本中每句话对应一个 group。注意：剧本某一行可能因极短句已并入相邻句而含多个句终标点，这种行**整行作为一个 group**，不要再按标点拆成两个 group。
...
- 不允许相邻两个 panel 的画面运动方式相同（固定画面不移动，不受此限制）
- panel 顺序按叙事时间排列
...
- 相邻两组之间，后一组第一个 panel 的景别以及画面运动不能与前一组最后一个 panel 相同
...
每行末尾的【画面：...；MG：...】包含同一 group 的两个上游意图：
- “画面”是原画 panels 的基础画面方向，必须据此做分镜。
- “MG”只供后续 MG HTML 标注与动画规划参考；不要把它写入 group text、image_prompt 或 video_prompt，也不要为了它改变原画分镜。
它们都只提供方向，不规定 panel 数量。
...
2. 抽象数学描述 → 纯黑背景数学模型
   论述属于抽象的数学/几何/函数关系（无对应现实产品或实物）时，用纯黑背景绘制便于读者理解的数学模型：
   坐标系与函数曲线、几何体、向量场、集合关系图等，平面或 3D 均可。
   线条与标注清晰准确，坐标轴和关键量标注真实变量名；不加装饰性光效（与「禁止元素」一致）。
3. 简约图示兜底
   只有当论述既无对应产品、也无对应实物，也不属于抽象数学关系时，才用扁平信息图：
...
- video_prompt：基于 image_prompt 画面的意象动态，英文，一律固定画面，以 "Fixed framing," 起头（不使用推近/拉远等画面运动）。不要重复 image_prompt 的静态描述，只补画面内意象的细微动态。例外：当 image_prompt 属于图表/统计/截图类（信息图、排行榜、对比柱状图、价格表、流程图、UI 截图、文档/界面截图等静态信息载体）时，**不输出 video_prompt 字段**——这类画面无动态，render 侧会用静态图填满时长，无需生成视频。数学模型类画面同理：仅当语义必须靠变量变化才体现（如"随 x 增大而收敛""曲面随参数形变"）时才输出 video_prompt，只描述该变量的连续变化；若模型本身静态即可说明问题，则不输出 video_prompt。

== group 字段 ==

- text：逐字照搬当前物理行中【画面：...；MG：...】之前的原文处理单元，不得改写或遗漏；不要把【】块写入 text
- panels：上述 panel 数组
...
- append_group 的 content 参数是单个 group 的 JSON 字符串，格式：
  {"text":"当前物理行中【】之前的原文处理单元","panels":[{"shot_type":"...","is_continuation":false,"image_prompt":"...","video_prompt":"..."}]}
...
const CLEAN_TEXT_SYSTEM = `你是原文预处理专员。任务：在进入分镜流程前，对小说章节或文章原文做"结构清理"，输出干净、便于后续逐句分镜的正文。

== 核心原则 ==
除下述明确允许的处理（展示型文字重构、错别字与明显病句订正）外，逐字保留原文，不得扩写、精简、增删剧情，不得改变事实、人物或对话原意。你只做结构清理与显错订正，不做内容改编或文风润色。
**保守优先**：若原文不含需处理的内容、且排版正常，就基本原样输出（至多剥离样板、订正显错）。不要为了"完成任务"而做激进或不必要的改动——宁可少改，不可错改。

== 处理规则 ==

1. 剥离非正文样板
仅删除作者署名、链接、来源、著作权/版权声明、风险提示和无关的页眉页脚等外围样板。保留小说正文或文章主体；产品介绍、项目介绍、观点表达、号召语等即使带有宣传性质，也属于文章主体，不得作为广告删除。
...
4. 正常叙事与对白原样保留
正常的叙述描写、人物对白、内心独白——不要合并、不要拆分，措辞与原意保持原样（仅可按第 5 条订正其中的错别字/病句）。一段连续的台词/独白本就是一个单元。
**例外**：极短独立句按第 4A 条并入相邻句。

4A. 极短独立句并入相邻句
- 判定：去掉标点后正文 ≤ 4 个汉字、且为完整独立句（如"没有。""天黑了。"他没说话。"），不得单独成句。
- 合并方向：并入叙事上紧贴、语义最连贯的相邻句（前后皆可）。
- 标点处理：把该短句的句终标点（。！？）替换为逗号，使其成为相邻句的一部分；相邻句本身的句终标点保留不变。
- 例：「翻遍了营地的每一个角落。没有。」→「翻遍了营地的每一个角落，没有。」
- 例外：第 3 条的展示型短文字（单字标题/招牌/字条）仍按画面保留，不合并；并入后语义明显受损或破坏必要停顿的短句保留原样（保守优先）。

5. 订正错别字与明显病句
- 改正明显的错别字、同音字、形近字、笔误（含 OCR/转录造成的串字、缺字、多字），以及读不通的明显语病（词序错乱、标点误用等）。
- 以「最小改动、恢复作者本意」为度：只动确属错误的字词，使句子通顺、语义正确即可，能不改就不改。
- 严禁借订正之名做内容改编：不得改变剧情、事实、人物或对话含义，不得润色文风、替换近义词、调整语气或增删信息。
- 拿不准是否为错误就保留原文（保守优先）。对白同样：只改错字，不改措辞与原意。
...
/** 在小说文件夹中按章号查找并读取原文，找不到返回 null（不抛错，供“下一章仅供参考”场景使用） */
async function loadChapterFileByNumber(novelFolder: string, chapterNum: number): Promise<string | null> {
  const filename = (await fs.readdir(novelFolder)).find(
    (f) => new RegExp(`^第${chapterNum}章`).test(f),
  );
...
async function loadRawChapter(novelName: string): Promise<string> {
  ...
  const text = await loadChapterFileByNumber(novelFolder, nextChapter);
  if (text == null) throw new Error(`找不到第${nextChapter}章文件`);
  return text;
}
...
export async function archive(sel: NovelSelection, visualPresetPath: string): Promise<ArchiveResult> {
  ...
  const readLaterChapterTool: ToolDefinition = {
    name: "read_later_chapter_text",
    label: "查阅后续章节原文",
    description: "读取本书更后面一章的原文（仅供参考，不为其中内容建档）。用于：1) 本章某角色只以代称/外貌出现、尚未点明真实姓名时，查看后续章节是否揭晓姓名；2) 判断本章某个造型是否会延续到后续章节。可连续调用查多个章节，直到找到需要的信息或章节不存在为止。",
    parameters: Type.Object({
      chapter: Type.Number({ description: `要查阅的章数（正整数），必须大于本章（第 ${sel.nextChapter} 章）` }),
    }),
    execute: async (_toolCallId: string, { chapter }: { chapter: number }) => {
      const ch = Math.trunc(chapter);
      if (!Number.isFinite(ch) || ch <= sel.nextChapter) {
        return {
          content: [{
            type: "text" as const,
            text: `ERROR: chapter 必须是大于 ${sel.nextChapter} 的整数（当前正在建档第 ${sel.nextChapter} 章）`,
          }],
          details: {},
        };
      }
      const text = await loadChapterFileByNumber(sel.sourcePath, ch);
      if (text == null) {
        return {
          content: [{ type: "text" as const, text: `ERROR: 第 ${ch} 章原文不存在（可能还没写到那，或已经查过头了），停止继续往后查，改用定语命名规则` }],
          details: {},
        };
      }
  ...
  const output = await runSubAgent(
    [resourceDetailTool, readLaterChapterTool],
    ARCHIVE_SYSTEM,
    taskPrompt,
    "[资源建档]",
...
// 阈值定得比视频模型的物理上限（约 40s）低很多，是节奏偏好而非仅为避免报错。
const CHARS_PER_SECOND = 5;
const MAX_PANEL_SECONDS = 20;
  ...
  const processFile = async (filename: string): Promise<void> => {
      ...
      const appendGroupTool: ToolDefinition = {
        ...
        execute: async (_toolCallId: string, { content }: { content: string }) => {
          ...
          // 1.5 时长校验：按 5 字/秒估算该 group 配音时长，均分给 panels 后
      ...
      // 人物身份判断不了时（如"我"对不上名单里哪个角色），给分镜 agent 一个按需查更早集数原文的工具，
      // 不强行把往期原文塞进每次的 prompt——多数场景身份明确，只有真遇到判断不了的情况才触发这次查询。
      const readPriorEpisodeTool: ToolDefinition = {
        name: "read_prior_episode_text",
        label: "查阅历史集原文",
        description: "读取本剧更早一集的原文。当角色名单和当前场景剧本都无法确定某个代词（如“我”“他”“她”）具体指代名单中哪个角色时使用——角色的真实姓名通常在较早的集数（尤其第 1 集）里被点名过，可借此确认后再标注。",
        parameters: Type.Object({
          episode: Type.Number({ description: "要查阅的集数（正整数），只能小于当前正在制作的集数" }),
        }),
        execute: async (_toolCallId: string, { episode }: { episode: number }) => {
          const ep = Math.trunc(episode);
          if (!Number.isFinite(ep) || ep < 1 || ep >= sel.episode) {
            return {
              content: [{
                type: "text" as const,
                text: `ERROR: episode 必须是 1 到 ${sel.episode - 1} 之间的整数（当前正在制作第 ${sel.episode} 集，不能查阅本集或更晚的集数）`,
              }],
              details: {},
            };
          }
          try {
            const text = await fs.readFile(novelPaths.cleanedText(sel.novelName, ep), "utf-8");
            return { content: [{ type: "text" as const, text }], details: {} };
          } catch {
            return {
              content: [{ type: "text" as const, text: `ERROR: 第 ${ep} 集原文不存在或尚未生成` }],
              details: {},
            };
          }
        },
...
export async function assignGlobalOrder(
  ...
  // 正向分 = LCS/len(group)：group 有多少按序出现在该 preset（主分，≥阈值才采纳）
  // 反向分 = LCS/len(preset)：preset 有多少被 group 占用（仅作并列裁决）
  ...
  for (const group of groups) {
    ...
    group.globalOrder = bestFwd >= MATCH_THRESHOLD ? bestMatch : 999;
    if (group.globalOrder === 999) {
      console.warn(`[assignGlobalOrder] 未匹配(最高正向 ${(bestFwd * 100).toFixed(0)}%): scene=${group.scene} gi=${group.gi} text="${group.text.slice(0, 60)}"`);
    }
  }
