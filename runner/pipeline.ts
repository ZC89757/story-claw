...
造型：角色在某段剧情里出现的、服装或随身道具发生显著变化的外观状态。
      「外观」只包含：服装、随身道具。情绪、动作、表情、姿态一律不算外观变化，不产生新造型。
      例：开学报到=背包拉行李箱；受伤后=坐轮椅；打架后=衣服脏破狼狈。
      反例（不生成）：热情推销、被吓跑、愤怒、哭泣——这些只是情绪/动作，外观未变。
  ...
  "new_characters": [
    {
      ...
      "stages": [
        { "stage": "造型阶段名", "prompt": "本阶段造型变化：服装、道具、特殊状态，需包含角色此时的年龄或人生阶段（如「约9岁」「青少年」「中年」）" }
      ]
    }
  ],
  "new_scenes": [
  ...
  "existing_character_stages": [
    {
      "name": "已有角色名",
      "stages": [
        { "stage": "新造型阶段名", "prompt": "本阶段造型变化，需包含角色此时的年龄或人生阶段（如「约9岁」「青少年」「中年」）" }
      ]
    }
  ],
...
2. 每个文件的保存路径必须与 task 中"输出路径"列表里的完整绝对路径完全一致，禁止使用相对路径或自创路径
3. 在最终回复末行写明：剧本目录: <保存的目录路径>
