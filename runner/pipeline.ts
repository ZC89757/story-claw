...
如果输入中包含「下一章原文」，那一段**仅供参考**，用于帮助你判断本章角色/场景的连续性，包括但不限于：
- 本章某个造型是否会延续到下一章、要不要提前按下一章的方向来定造型
- 本章角色的真实姓名：角色名会作为图片资源的文件名跨章节共享（如 characters/{name}_原型.png）。若本章某角色尚未点明姓名（只以代称、外貌或身份描述出现），但下一章参考原文里揭晓了这个角色的真实姓名，建档时必须直接用真实姓名，不要用代称或描述性词语当作 name——否则下一章会因为名字对不上被判定为「不在已有列表」，重复建一次角色、生成重复的图片资源，还会打乱音色分配

只为「本章」原文中出现的角色和场景建档；下一章原文中新出现、本章完全没提到的角色或场景，不要建档。

角色全程无真实姓名时，name 字段用一个简短定语短语代替，不要用"路人""老妇人"这种毫无区分度的泛称：
...
export async function archive(sel: NovelSelection, visualPresetPath: string): Promise<ArchiveResult> {
  ...
  // 下一章原文仅作连续性参考传给 LLM，不在本章建档范围内；最后一章时不存在，静默跳过
  const nextChapterText = await loadChapterFileByNumber(sel.sourcePath, sel.nextChapter + 1);
  ...
  const taskPrompt = [
    ...
    ``,
    ...(nextChapterText ? [
      `== 下一章原文（仅供参考，帮助判断角色/场景连续性，不要为其中本章未出现的新内容建档）==`,
      nextChapterText,
      ``,
    ] : []),
    `== 已有资源 ==`,
